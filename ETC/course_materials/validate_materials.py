...
Runtime tests are separate: python ETC/course_materials/run_validation.py --suite all ...
"""
import argparse
import ast
import copy
...
EXCLUDE = {".git", ".venv", ".ipynb_checkpoints", "__pycache__", ".pytest_cache", "validation-runs", "runs", "outputs"}


def relocated_asset(relative):
    mapping = json.loads((ROOT / "ETC/course_materials/layout.json").read_text())["paths"]
    for old in sorted(mapping, key=len, reverse=True):
        if relative == old or relative.startswith(old + "/"):
            return mapping[old] + relative[len(old):]
    return relative
...
def notebook_output_messages(notebook, relative, allow_executed=False):
    """Inspect saved results without deleting or certifying a learner's work."""
    errors, warnings = [], []
    for number, cell in enumerate(notebook["cells"], 1):
        if cell["cell_type"] == "code" and cell.get("outputs"):
            location = f"{relative} cell {number}"
            if allow_executed:
                warnings.append("saved notebook outputs preserved, not validated: " + location)
            else:
                errors.append("clear stale notebook outputs: " + location)
...
def main():
    ...
    report = {"scope": "static source, full course coverage, notebooks, local links, schedule and assets",
              ...
              "python": [], "notebooks": [], "links": [], "assets": [], "errors": [], "warnings": [],
              "saved_outputs_policy": "preserve_without_validation" if args.allow_executed_notebooks else "require_clean",
    ...
    if tree.returncode == 0:
        for entry in tree.stdout.splitlines():
            ...
            if Path(relative).suffix.lower() in {".py", ".yaml", ".yml", ".md", ".ipynb"}:
                continue
            path = ROOT / relocated_asset(relative)
        ...
        check(len(report["assets"]) == 45, "expected 45 preserved upstream assets")
    ...
    for path in sorted(p for p in ROOT.rglob("*.ipynb") if is_active(p)):
        ...
        try:
            ...
            output_errors, output_warnings = notebook_output_messages(notebook, relative, args.allow_executed_notebooks)
    ...
    course = (ROOT / "ETC/course_materials/course-plan.md").read_text(encoding="utf-8")
    ...
    for line in course.splitlines():
        ...
        if row:
            ...
            check(end - start == minutes and minutes > 0, f"8h schedule: interval/duration mismatch: {line}")
            check(previous_end is None or previous_end == start, f"8h schedule: gap/overlap before {line}")
            previous_end = end
    ...
    report["schedules"] = {"8": schedule}

    docker = (ROOT / "ETC/environment/Dockerfile").read_text(encoding="utf-8")
    ...
    print(json.dumps({"status": report["status"], "python": len(report["python"]), "notebooks": len(report["notebooks"]), "links": len(report["links"]), "coverage": report["coverage"], "errors": report["errors"], "warnings": report["warnings"]}, ensure_ascii=False, indent=2))
