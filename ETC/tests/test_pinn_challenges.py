...
import torch
from ETC.runtime.pinn import create_model, create_informer, residuals, gradient, heldout_losses

# Numbered lesson directories are importable through importlib, not import syntax.
wave_l1, wave_l2, wave_l3 = (
    importlib.import_module(f"02_challenges.01_wave.wave_l{level}") for level in (1, 2, 3)
)
chip_2d_l1, chip_2d_l2, chip_2d_l3 = (
    importlib.import_module(f"02_challenges.02_fluid.chip_2d_l{level}") for level in (1, 2, 3)
)
fluid_geometry = importlib.import_module("02_challenges.02_fluid.fluid_geometry")
SINGLE_BLOCK, THREE_BLOCKS = fluid_geometry.SINGLE_BLOCK, fluid_geometry.THREE_BLOCKS
outside_blocks = fluid_geometry.outside_blocks
sample_interior, sample_walls = fluid_geometry.sample_interior, fluid_geometry.sample_walls
sample_flux, inlet_velocity = fluid_geometry.sample_flux, fluid_geometry.inlet_velocity
climate_l1, climate_l2 = (
    importlib.import_module(f"02_challenges.03_climate.climate_l{level}") for level in (1, 2)
)
...
class ClimatePhysicsTests(unittest.TestCase):
    ...
    def test_coupling_equal_and_opposite_and_reference_scope(self):
        ...
        self.assertEqual(climate_l2.exact_reference(xy, time, params).shape, (3, 2))
        self.assertIsNone(climate_l2.exact_reference(xy, time, {**params, "Q_a0": 1.}))
