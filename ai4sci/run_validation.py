...
MANIFEST = ROOT / "ai4sci/course_manifest.json"
...
def source_snapshot(output):
    ...
    try:
        ...
        paths = [path for folder in ("tutorial", "challenge", "ai4sci", "tests")
    ...
    for path in sorted(set(paths)):
        if not path.is_file() or output == path or output in path.parents or any(p.startswith("._") for p in path.parts):
            continue
        if any(part in {".git", "__pycache__", ".venv", "venv", "outputs", ".pytest_cache"}
...
def validate_artifacts(path, *, steps, device, expected_version, seed):
    ...
    if len(rows) < 2 or not loss_columns:
...
def prepare_operator_data(output, suite, seed, timeout):
    ...
    command = [sys.executable, str(ROOT / "challenge/neural_operator/generate_data.py"),
...
def main(argv=None):
    ...
    try:
        ...
        if args.suite in ("unit", "all"):
            unit = execute([sys.executable, "-m", "pytest", "tests", "-q", "--tb=short",
