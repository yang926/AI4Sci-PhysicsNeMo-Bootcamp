"""Level 1: Constant-speed 2D wave. PhysicsNeMo 2.2.2 explicit-loop PINN.
...
def student_equations(x, y, t, u, c=1.0):
    ...
    # Both initial displacement and initial velocity are sin(x)*sin(y).
...
def reference_conditions(x, y, t, u):
    return {"initial_u": sin(x) * sin(y),
            "initial_ut": sin(x) * sin(y), "boundary": u}
...
def exact_reference(coordinates, time, c=1.0):
    omega = math.sqrt(2) * c
    return initial_displacement(coordinates) * (torch.cos(omega * time) + torch.sin(omega * time) / omega)
...
def main():
    args, config = parse_args(__file__, __doc__, "config_wave_l1.yaml")
    ...
    save_results(args, config, model, history, xy, time, FIELD_NAMES, metrics,
                 ...
                 'Corrected c=1 exact solution retains both original sin(x)sin(y) initial conditions.'])
