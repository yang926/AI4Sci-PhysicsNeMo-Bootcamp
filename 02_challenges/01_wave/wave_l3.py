...
def student_equations(x, y, t, u, c=1.0):
    ...
    # Here c=1. Implement the Robin boundary separately in student_conditions.
...
def student_speed(x, y):
    # FIXME: return {"c": ...} for the constant speed c=1.
...
def reference_conditions(x, y, t, u):
    return {"initial_u": exp(-20 * ((x - .3)**2 + y**2)) + exp(-20 * ((x + .3)**2 + y**2)),
            "initial_ut": 0, "boundary": u + .5 * (x * u.diff(x) + y * u.diff(y))}


def student_conditions(x, y, t, u):
    # FIXME: return initial_u, initial_ut, and the boundary residual (target zero).
    # Unit-circle outward normal is (x,y): construct the Robin residual, not u=0.
...
def initial_displacement(coordinates):
    """Original two Gaussian pulses; the initial Robin compatibility is not exact."""
...
def main():
    ...
    save_results(args, config, model, history, xy, time, FIELD_NAMES, metrics,
                 ...
                 'Original Gaussian IC restored; it is not exactly compatible with Robin at t=0. No closed-form reference is asserted; PDE and condition residuals do not replace independent solution validation.'])
